...
async function paymentGate(env,bookingId,status){const payment=await paymentStore(env).getByBooking(bookingId),refund=await paymentStore(env).getRefund(bookingId);if(['berlangsung','selesai'].includes(status)&&payment?.status!=='COMPLETED')return {ok:false,error:'payment_required',paymentStatus:payment?.status||'UNPAID'};if(['dibatalkan','ditolak'].includes(status)&&payment?.status==='COMPLETED'){if(refund?.status==='SUCCEEDED')return {ok:true};if(refund?.status==='PENDING')return {ok:false,error:'refund_pending',refundStatus:'PENDING'};return {ok:false,error:'refund_required_before_cancel',paymentStatus:'COMPLETED'}}return {ok:true}}
...
async function handleProtected(request,env){
 ...
 if(path.startsWith('/api/payments/')){const payment=await handlePaymentApi(request,env,user);if(payment)return payment}
 if(path==='/api/bookings'&&method==='POST'){
  if(!['customer','admin'].includes(user.role))return json({ok:false,error:'customer_role_required'},403);
  const submitted=await request.clone().json().catch(()=>({}));const response=await bookingWorker.fetch(request,env);
  if(response.ok){try{const data=await response.clone().json();if(data?.booking?.id){if(user.role==='customer')await authStore(env).bindBooking(user.id,data.booking.id,'customer');if(submitted.partnerUserId)await authStore(env).assignPartner(data.booking.id,String(submitted.partnerUserId))}}catch{}}
  return response;
 }
 if(path==='/api/bookings'&&method==='GET'){if(user.role==='admin')return bookingWorker.fetch(request,env);const ids=await authStore(env).listBookingIds(user.id,user.role);const s=bookingStore(env),bookings=[];for(const id of ids.slice(0,100)){const b=await s.getBooking(id);if(b)bookings.push(b)}return json({ok:true,bookings})}
 if(path==='/api/transactions'&&method==='GET'){const id=url.searchParams.get('bookingId')||'';if(!(await canAccess(user,env,id)))return json({ok:false,error:'forbidden'},403);return bookingWorker.fetch(request,env)}
 const id=bookingIdFromPath(path);
 if(id){
  if(!(await canAccess(user,env,id)))return json({ok:false,error:'forbidden'},403);
  const isBase=/^\/api\/bookings\/[^/]+$/.test(path),isReschedule=/^\/api\/bookings\/[^/]+\/reschedule$/.test(path),isReview=/^\/api\/bookings\/[^/]+\/review$/.test(path);
...
export default {async fetch(request,env){const path=new URL(request.url).pathname;if(path==='/api/webhooks/xendit/payment-session'||path==='/api/webhooks/xendit'){const payment=await handlePaymentApi(request,env,null);if(payment)return payment}const auth=await handleAuthApi(request,env);if(auth)return auth;if(path==='/api/directory'||/^\/api\/partners\/[^/]+$/.test(path)){const publicPartner=await handlePartnerApi(request,env,null);if(publicPartner)return publicPartner}const protectedResponse=await handleProtected(request,env);if(protectedResponse)return protectedResponse;return bookingWorker.fetch(request,env)}};
