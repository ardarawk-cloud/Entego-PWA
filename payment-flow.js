...
async function renderPaymentCard(){
 ...
 try{
  ...
  if(!cfg.d.configured){card.innerHTML=`<div class="kicker">PAYMENT</div><h3 style="margin:6px 0">${payMoney(order.total)}</h3><span class="pill blue">Xendit Sandbox belum dikonfigurasi</span><p class="meta">Booking tersimpan aman. Pembayaran akan aktif setelah secret Xendit dipasang di Cloudflare.</p>`;return}
  const st=p?.status||'UNPAID',done=st==='COMPLETED';
 ...
 }catch{card.innerHTML='<div class="kicker">PAYMENT</div><span class="pill blue">Gateway belum tersambung</span><p class="meta">Booking tetap tersimpan. Coba pembayaran lagi setelah backend production aktif.</p>'}
}
...
const payRun=()=>renderPaymentCard();new MutationObserver(payRun).observe(document.documentElement,{childList:true,subtree:true});window.addEventListener('load',()=>{handlePaymentReturn();payRun()});handlePaymentReturn();payRun();
