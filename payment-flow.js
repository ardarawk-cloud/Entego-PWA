...
async function createPaymentSession(button){
 ...
 try{const {r,d}=await payJson('/api/payments/session',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({bookingId:order.id})});if(!r.ok||!d.ok)throw new Error(d.error||'payment_failed');if(d.payment?.status==='COMPLETED'){document.querySelector('#entegoPaymentCard')?.remove();renderPaymentCard();return}if(!d.payment?.paymentLinkUrl)throw new Error('payment_link_missing');location.assign(d.payment.paymentLinkUrl)}catch(e){button.disabled=false;button.textContent=old;alert(e.message==='payment_not_configured'?'Xendit Sandbox belum dikonfigurasi di Cloudflare.':'Sesi pembayaran belum dapat dibuat. Coba lagi.')}
}
