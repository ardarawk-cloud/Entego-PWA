...
export async function handlePartnerApi(request,env,user){
 ...
 if(path==='/api/directory'&&method==='GET')return json({ok:true,partners:await s.listDirectory({q:url.searchParams.get('q')||'',category:url.searchParams.get('category')||'',area:url.searchParams.get('area')||'',maxPrice:url.searchParams.get('maxPrice')||0,limit:url.searchParams.get('limit')||50})});
