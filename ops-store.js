...
export class EntegoOps extends DurableObject{
 ...
 async openDispute(bookingId,user){const id=clean(bookingId,120),uid=clean(user?.id,100),role=clean(user?.role,20),note=clean(user?.note,2000);if(!id||!uid||!note)throw new Error('DISPUTE_REQUIRED');const current=await this.getOpenDispute(id);if(current)return current;const now=new Date().toISOString(),disputeId=`DSP-${Date.now()}-${crypto.randomUUID().slice(0,6).toUpperCase()}`;this.sql.exec(`INSERT INTO disputes(id,booking_id,opened_by,opener_role,note,status,resolution,created_at,updated_at) VALUES(?,?,?,?,?,'open','',?,?)`,disputeId,id,uid,role,note,now,now);const c=await this.getCompletion(id);if(c)this.sql.exec(`UPDATE booking_completion SET status='disputed',updated_at=? WHERE booking_id=?`,now,id);return this.getDispute(disputeId)}
