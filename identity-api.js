...
const IDENTITY_TYPES=['KTP','SIM','PASSPORT'];
...
function identityError(error){
 ...
 const status=new Map([
  ...
  ['INVALID_IDENTITY_TYPE',400],['IDENTITY_DOCUMENT_REQUIRED',400],['FULL_ID_NUMBER_NOT_ACCEPTED',400],['FULL_BANK_NUMBER_NOT_ACCEPTED',400]
 ]).get(code)||500;
...
async function partnerIdentity(request,env,user){
 ...
 if(request.method==='GET')return json({ok:true,identity:await s.getIdentity(user.id),privateDocumentStorageConfigured:identityStorageConfigured(env),identityTypes:IDENTITY_TYPES,dataPolicy:{fullNikStored:false,fullBankAccountStored:false,documentsPublic:false}});
 if(request.method==='PUT'){
  const blocked=await rate(env,user,'identity-details',20,3600);if(blocked)return blocked;
  const body=await request.json().catch(()=>({}));
  try{return json({ok:true,identity:await s.saveIdentity(user.id,body),privateDocumentStorageConfigured:identityStorageConfigured(env),identityTypes:IDENTITY_TYPES})}catch(e){return identityError(e)}
 }
...
async function uploadDocument(request,env,user){
 ...
 const url=new URL(request.url),requestedKind=clean(url.searchParams.get('kind'),20);if(!['ktp','identity','selfie'].includes(requestedKind))return json({ok:false,error:'invalid_identity_document_kind'},400);
 const kind=requestedKind==='identity'?'ktp':requestedKind;
 ...
 const s=partnerStore(env),old=await s.getIdentityDocumentRef(user.id,kind),ext=ALLOWED_IDENTITY_MEDIA.get(file.type),storageKind=kind==='ktp'?'identity':kind,key=`identity/${user.id}/${storageKind}/${Date.now()}-${crypto.randomUUID()}.${ext}`;
 try{
  await env.ENT_IDENTITY_MEDIA.put(key,file.stream(),{httpMetadata:{contentType:file.type,cacheControl:'private, no-store'},customMetadata:{owner:user.id,kind:storageKind,classification:'restricted-identity',originalName:clean(file.name,180)}});
  ...
  return json({ok:true,identity,document:{kind:storageKind,uploaded:true,size,contentType:file.type}},201);
...
async function adminIdentityPartners(request,env,user){
 ...
 return json({ok:true,partners,privateDocumentStorageConfigured:identityStorageConfigured(env),identityTypes:IDENTITY_TYPES,policy:{approvalRequiresIdentitySubmission:true,payoutRequiresIdentityApproval:true,fullNikStored:false,fullBankAccountStored:false}});
}
...
async function adminDocument(request,env,user,userId,requestedKind){
 if(user?.role!=='admin')return json({ok:false,error:'admin_required'},403);
 if(!identityStorageConfigured(env))return json({ok:false,error:'identity_private_storage_not_configured'},503);
 const kind=requestedKind==='identity'?'ktp':requestedKind;
 ...
 const label=kind==='ktp'?'identity':kind;
 const headers=new Headers({'content-type':ref.contentType||'application/octet-stream','cache-control':'private, no-store, max-age=0','pragma':'no-cache','x-content-type-options':'nosniff','content-disposition':`inline; filename="entego-${label}-${clean(userId,40)}"`});
...
async function reviewPartnerIdentity(request,env,admin,userId){
 ...
 try{
  ...
  if(action==='approve')identity=await s.reviewIdentity(userId,'approve',admin.id,note||'Dokumen identitas dan rekening diperiksa Admin ENTEGO.');
...
export async function handleIdentityApi(request,env){
 ...
 if(path==='/api/identity/config'&&request.method==='GET')return json({ok:true,privateDocumentStorageConfigured:identityStorageConfigured(env),maxBytes:MAX_IDENTITY_BYTES,accept:[...ALLOWED_IDENTITY_MEDIA.keys()],identityTypes:IDENTITY_TYPES,fullNikStored:false,fullBankAccountStored:false,documentsPublic:false,payoutGate:'identity-approved-required'});
 ...
 m=path.match(/^\/api\/admin\/identity\/partners\/([^/]+)\/document\/(ktp|identity|selfie)$/);if(m&&request.method==='GET')return adminDocument(request,env,user,decodeURIComponent(m[1]),m[2]);
